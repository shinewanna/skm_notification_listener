...
import 'skm_notification_listener.dart';
...
class ServiceNotificationEvent {
  ...
  ///
  /// ```
  /// Image.memory(notification.extrasPicture)
  /// ```
  ...
  ///
  /// ```
  /// Image.memory(notification.appIcon)
  /// ```
  Uint8List? appIcon;
  ...
  ///
  /// ```
  /// Image.memory(notification.largeIcon)
  /// ```
  Uint8List? largeIcon;
  ...
  Future<bool> sendReply(String message) async {
    if (!canReply!) throw Exception("The notification is not able to reply");
    try {
      return await methodChannel.invokeMethod<bool>("sendReply", {
            'message': message,
            'notificationId': id,
          }) ??
          false;
